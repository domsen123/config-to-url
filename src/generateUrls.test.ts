...
describe('generateUrls', () => {
  ...
  it('handles large config', () => {
    ...
    const config: Config[] = [
      // Multiple params
      { data: ['param1Value1', 'param1Value2', 'param1Value3'], currentIndex: 0, assignTo: 'param', assignToKey: 'param1' },
      { data: ['param2Value1', 'param2Value2', 'param2Value3'], currentIndex: 0, assignTo: 'param', assignToKey: 'param2' },
      // ... add more params as needed
      // Multiple queries
      { data: ['query1Value1', 'query1Value2'], currentIndex: 0, assignTo: 'query', assignToKey: 'query1' },
      { data: ['query2Value1', 'query2Value2'], currentIndex: 0, assignTo: 'query', assignToKey: 'query2' },
    ...
    const expectedNumberOfUrls = config.filter(c => c.assignTo === 'param').reduce((acc, curr) => acc * curr.data.length, 1)
      * config.filter(c => c.assignTo === 'query').reduce((acc, curr) => acc * curr.data.length, 1)
